...
class Mutator:
    ...
    def __init__(self):
        ...
        self.api_constraint_dict = {}
        self.layer_constraint_dict = {}

        path_api = file_paths.API_INFO_PATH
        file_list = os.listdir(path_api)
        for file in file_list:
            if 'jittor.nn.' not in file:
                continue
            else:
                with open(os.path.join(path_api, file), 'r', encoding='utf-8') as f:
                    content = yaml.full_load(f.read())
                    f.close()
                self.api_constraint_dict[file[:-5]] = content
        path_layer = file_paths.LAYER_INFO_PATH
        file_list_2 = os.listdir(path_layer)
        for file in file_list_2:
            if 'jittor.nn.' not in file:
                continue
            else:
                with open(os.path.join(path_layer, file), 'r', encoding='utf-8') as f:
                    content = yaml.full_load(f.read())
                    f.close()
                self.layer_constraint_dict[file[:-5]] = content
