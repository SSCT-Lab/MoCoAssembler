...
class InceptionAux(nn.Module):
    ...
    def execute(self, x):
        ...
        x = jt.flatten(x, 1)
        nn.dropout(x, 0.5, )
        x = nn.dropout(x, 0.5, is_train=self.is_training())
        x = self.fc1(x)
        x = self.relu(x)
...
class GoogLeNet(nn.Module):
    ...
    def execute(self, x):
        ...
        x = self.dropout(x)
        x = self.fc(x)
        if self.is_training() and self.aux_logits:
            return x, aux2, aux1
        return x


if __name__ == '__main__':
    net = GoogLeNet(1000, aux_logits=False)
    summary(net, (3, 224, 224))
    net2 = GoogLeNet(1000, aux_logits=True)
    summary(net2, (3, 224, 224))
    xin = jt.randn((3, 3, 224, 224))
    yout = net(xin)
    yout2 = net2(xin)
