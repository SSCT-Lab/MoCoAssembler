...
class MobileNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 9th block
        x = self.relu(self.conv8a(x))
        x = self.relu(self.conv8b(x))

        # 10th block
        x = self.relu(self.conv9a(x))
        x = self.relu(self.conv9b(x))

        # 11th block
        x = self.relu(self.conv10a(x))
        x = self.relu(self.conv10b(x))

        # 12th block
        x = self.relu(self.conv11a(x))
        x = self.relu(self.conv11b(x))

        # 13th block
        x = self.relu(self.conv12a(x))
        x = self.relu(self.conv12b(x))

        # 14th block
        x = self.relu(self.conv13a(x))
        x = self.relu(self.conv13b(x))
