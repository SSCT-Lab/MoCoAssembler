...
# googlenet hidden layer
    ...
    x = keras.layers.MaxPool2D(pool_size=6, strides=2, padding="same")(x)
    x = inception_1(x, 64, 96, 128, 16, 32, 32)
    x = inception_2(x, 128, 128, 192, 32, 96, 64)
    x = keras.layers.LayerNormalization(axis=[-1], epsilon=0.0)(x)
...
def inception_1(inputs, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj):
    x1 = keras.layers.Conv2D(filters=ch1x1, kernel_size=1, activation="relu", padding="valid")(inputs)
    ...
    x4 = keras.layers.MaxPool2D(pool_size=3, strides=4, padding="same")(inputs)
    x4 = keras.layers.Conv2D(filters=pool_proj, kernel_size=1, strides=1, padding="valid", activation="relu")(x4)
...
def inception_2(inputs, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj):
    x1 = keras.layers.Conv2D(filters=ch1x1, kernel_size=1, activation="relu", bias_constraint=None)(inputs)
    ...
    x4 = keras.layers.ZeroPadding2D(padding=1)(x4)
