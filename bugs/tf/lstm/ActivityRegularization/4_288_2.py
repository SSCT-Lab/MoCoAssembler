...
# lstm hidden layer
    x = keras.layers.Embedding(input_dim=100, output_dim=10, input_length=8, embeddings_regularizer=None)(input_tensor)
    x = keras.layers.LSTM(units=16, activation='sigmoid', return_state=False)(x)
    x = keras.layers.Dropout(rate=0.3, noise_shape=None)(x)
    x = keras.layers.ActivityRegularization(l2=0.6474391647779326, l1=-0.23986406426077322)(x)
# lstm output layer
