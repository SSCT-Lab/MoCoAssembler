...
# lenet hidden layer
    ...
    x = keras.layers.MaxPool2D(pool_size=(4, 4), strides=2)(x)
    x = keras.layers.MaxPool2D(strides=1, padding="valid")(x)
    x = keras.layers.AveragePooling2D(pool_size=2, strides=(1, 1))(x)
    x = keras.layers.Conv2D(filters=32, kernel_size=5, activation="relu", padding="same", kernel_regularizer=None)(x)
