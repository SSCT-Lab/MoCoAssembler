...
# googlenet hidden layer
    ...
    x = keras.layers.MaxPool2D(pool_size=3, strides=3, padding="same")(x)
    x = keras.layers.MaxPool2D(strides=2, padding="same")(x)
    x = keras.layers.Conv2D(filters=192, kernel_size=3, strides=1, padding="same", activation="relu", use_bias=True)(x)
    x = keras.layers.MaxPool2D(pool_size=3, strides=6, padding="same")(x)
    x = inception_1(x, 64, 96, 128, 16, 32, 32)
    x = inception_2(x, 128, 128, 192, 32, 96, 64)
    x = keras.layers.LayerNormalization(scale=True, epsilon=100.0)(x)
...
def inception_1(inputs, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj):
    x1 = keras.layers.Conv2D(filters=ch1x1, kernel_size=1, activation="relu", dilation_rate=(3, 4))(inputs)
    ...
    x4 = keras.layers.MaxPool2D(pool_size=3, strides=(3, 2), padding="same")(inputs)
    x4 = keras.layers.Conv2DTranspose(filters=pool_proj, kernel_size=1, strides=1, padding="valid", activation="relu")(x4)
...
def inception_2(inputs, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj):
    x1 = keras.layers.Conv2D(filters=ch1x1, kernel_size=1, activation="relu", dilation_rate=(4, 3))(inputs)
    ...
    x4 = keras.layers.MaxPool2D(pool_size=3, strides=(5, 6), padding="same")(inputs)
    x4 = keras.layers.DepthwiseConv2D(kernel_size=1, strides=(3, 4), padding="same", activation="relu")(x4)
