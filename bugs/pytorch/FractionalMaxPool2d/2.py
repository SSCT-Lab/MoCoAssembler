...
class googlenet(nn.Module):
    def __init__(self):
        ...
        self.layer3 = torch.nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True, return_indices=False)
        self.layer4 = torch.nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1, stride=1, dilation=6)
        self.layer5 = torch.nn.ReLU(inplace=False)
        self.layer6 = torch.nn.AdaptiveAvgPool2d(output_size=1)
        self.layer7 = torch.nn.Hardsigmoid(inplace=False)
        self.layer8 = torch.nn.FractionalMaxPool2d(kernel_size=3, output_ratio=0.8654403995523432, return_indices=False)
