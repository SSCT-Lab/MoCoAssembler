...
class Filter:
    def __init__(self):
        self.info_lis = []
        self.tkg = None
        self.addkey(r"in_channels must be divisible by groups")
        self.addkey(r"out_channels must be divisible by groups")
        self.addkey(r"Expected .*? input to .*? but got input of size")
        self.addkey(r"Dimension out of range \(expected to be in range of \[.*?\], but got .+?\)")
        self.addkey("'tuple' object has no attribute")

        self.addkey("assert a.shape[-1] == b.shape[-1], (a.shape, b.shape)")
        self.addkey(r"AssertionError: \(\[[^\]]*\], \[[^\]]*\]\)")
        self.addkey(r"ValueError: not enough values to unpack \(expected \d+, got \d+\)")
...
class TreeNode:
    ...
    def run(self):
        ...
        if len(self.getModel().graph) != 0:
            block = self.getModel().graph[-1]
            if "MaxPool" in block.apiName and "AdaptiveMaxPool" not in block.apiName and self.father is not None:
            ...
            if "AvgPool" in block.apiName and "AdaptiveAvgPool" not in block.apiName and self.father is not None:
                kernel_size = block.params.get("kernel_size", None)
                previous_output_shape = self.father.outputShape
                kernel_size_height = kernel_size if isinstance(kernel_size, int) else kernel_size[0]
                ...
                if kernel_size_height > previous_output_shape_height or kernel_size_width > previous_output_shape_width:
                    print(
                        f"Skip running: kernel_size {kernel_size} is larger than previous output {previous_output_shape}.")
    ...
    def preCheck(self, block):
        code = f"import jittor\n" \
               f"import jittor.nn as nn\n" \
               f"\n" \
               f"\n" \
               f"def pre_check():\n" \
               f"    x = jittor.randn({str(self.outputShape)})\n" \
...
class Assembler:
    ...
    def startAGen(self, passedLastGenTreeNodes: list[TreeNode], block, gen):
        ...
        count = 1
        currentGenTreeNodes = []
        with alive_bar(self.n * len(passedLastGenTreeNodes), bar="filling", spinner="classic",
                       title=f"{self.seedName}-{gen}") as bar:
