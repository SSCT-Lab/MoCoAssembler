...
class Parser:
        ...
        while self.HasNextLine():
            ...
            if not self.in_class:
                ...
                if '__init__' in currentLine:
                    ...
                    self.in_forward = False
                    buffer.clear()
                elif 'def execute' in currentLine:
                    self.ParseDeclaration(buffer)
                    buffer.clear()
                    self.in_init = False
                    self.in_forward = True
                    model_inputs = [var.strip() for var in
                                    currentLine[currentLine.find('(') + 1:currentLine.find(')')].split(',')]
    ...
    def ParseDeclarationStatement(self, declarationStatement: str):
        if declarationStatement == "":
            return
        pattern = r"self\.(\w+)\s*=\s*(jittor\.nn\.\w+|Inception)\((.*?)\)$"
        match = re.match(pattern, declarationStatement.strip())
        pattern2 = r"self\.(\w+)\s*=\s*(jittor\.\w+)"
        match2 = re.match(pattern2, declarationStatement.strip())
        if (not match) and (not match2):
            print(f"Invalid model definition format, declarationStatement: {declarationStatement}")
            return
        if match:
    ...
    def ParseForwardStatement(self, forwardStatement: str):
        ...
        if not match:
            print(f"Invalid model definition format, forwardStatement: {forwardStatement}")
            return
...
if __name__ == "__main__":
    ...
    # p = Parser("")
    #
    # decl = "self.conv_1 = jittor.nn.Conv2d(in_channels=1, out_channels=6, kernel_size=(5, 5))"
