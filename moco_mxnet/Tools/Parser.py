...
def GetSeed(seedName):
    p = Parser(filePath=f"./Data/MXNet/seed_models/{seedName}.py")
    a = p.Parse(seedName)
    a.ModelPreHandle()
    return a
...
class Parser:
    ...
    def ParseDeclarationStatement(self, declarationStatement: str):
        if declarationStatement == "":
            return
        pattern = r"self\.(\w+)\s*=\s*(mxnet\.gluon\.nn\.\w+|Inception)\((.*?)\)$"
        match = re.match(pattern, declarationStatement.strip())
        pattern2 = r"self\.(\w+)\s*=\s*(mxnet\.\w+)"
...
if __name__ == "__main__":
    models = []
    for model in ["lenet", "alexnet", "pointnet", "squeezenet", "LSTM"]:
    ...
    #     fileName="t"
    # )
    seeds = ["lenet", "squeezenet", "pointnet", "alexnet", "LSTM"]
    for seed in seeds:
        a = GetSeed(seed)
        a.AssembleFile(
            "../Test",
