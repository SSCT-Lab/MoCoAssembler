...
class Departed_Model:
    def __init__(self, model_name: str):
        ...
        if model_name in  ['resnet18',
            ...
            self.get_transformer_model_from_file(model_name)

    def assemble_file(self, generation: int = 0, index: int = 1, extra_index: int = 0, pth: str = None) -> str:
        if pth is None:
            path = os.path.join(file_paths.MUTATED_MODEL_PATH, self.net_name)
        else:
            path = pth
        if extra_index == 0:
            file_name = self.net_name + '-' + str(generation) + '-' + str(index) + '.py'
        else:
            file_name = self.net_name + '-' + str(generation) + '-' + str(index) + '-' + str(extra_index) + '.py'
        f = open(os.path.join(path, file_name), 'w', encoding='utf-8')
        f.write(self.begin)
        f.close()
        f = open(os.path.join(path, file_name), 'a', encoding='utf-8')
    ...
    def get_model_from_file(self, model_name: str) -> None:
        ...
        for index, line in enumerate(lines):  # Go to main model analysis
            if 'nn.Module' in line:
                begin_index = index
                end_index = index + 1
                while 'nn.Module' not in lines[end_index] and 'def go()' not in lines[end_index]:
                ...
                if 'init' in lines[i]:
                    ...
                    while 'def' not in lines[index]:
                        if 'self.' in lines[index] and 'Seq' not in lines[index]:  # analyse this line in dict
                            ...
                            index = index + 1
                            seq_list = []
                            while 'torch.nn.' in lines[index] or 'ConvBNReLU' in lines[index] or 'InceptionV3Module' in lines[index] or 'SeparableConv2d' in lines[index]:
    ...
    def get_transformer_model_from_file(self, model_name: str) -> None:
        temp_list = []
        with open(os.path.join(file_paths.MODEL_PATH, model_name + '.py'), 'r', encoding='utf-8') as f:
            lines = f.readlines()
        ...
        while index < len(lines):
            ...
            if '(nn.Module)' in line:
                ...
                while '(nn.Module)' not in lines[end_index] and 'def go()' not in lines[end_index]:
                    end_index += 1
                class_code_block = lines[begin_index:end_index]
                model: Single_Model = self.single_transformer_model_analyse(class_code_block)
                if first_block_flag:
                    first_block_flag = False
                    self.begin = connect_str_list(temp_list)
                    temp_list.clear()
                if 'Transformer(nn.Module)' in line:
                    self.main_model = model
                else:
                    self.block_dict[model.model_name] = model
                index = end_index
    ...
    def single_transformer_model_analyse(self, line_list) -> Single_Model:
        # input: a line list starts with 'class' and ends with 'return x', as a single model
        # output: a standard single model
        # format: this input line list should be of special format
        lines = copy.deepcopy(line_list)
        ...
        while i < n:
            ...
            if 'def' in lines[i]:
                if 'init' in lines[i]:
                    index = i
                    init_lines = [lines[index]]
                    index = index + 1
                    while 'def' not in lines[index]:
                        if 'self.' not in lines[index] or "self.register_buffer" in lines[index]:
                            if lines[index] != "\n":
                                init_lines.append(lines[index])
                        elif 'self.' in lines[index]:
                            line = lines[index].replace(' ', '').replace('\n', '')
                            name = line.split('=', 1)[0][5:]
                            declare = line.split('=', 1)[1]
                            result.declaration[name] = declare
                        elif 'self.' in lines[index] and "nn.ModuleList" in lines[index]:
                            line = lines[index].replace('\n', '')
                            name = line.split('=', 1)[0][5:]
                            declare = line.split('=', 1)[1]
                            result.declaration[name] = declare
                        index = index + 1
                    result.init_sentence = ''.join(init_lines)
                    i = index
                elif 'forward' in lines[i]:
                    index = i
                    result.execute_sentence = copy.deepcopy(lines[index])
...
if __name__ == '__main__':
    transformer = Departed_Model('transformer')
    # print(transformer.block_dict["PositionalEncoding"].assemble())
    print(transformer.main_model.assemble())
