...
class _TransitionLayer(torch.nn.Module):
    def __init__(self, inplace, plance):
        super(_TransitionLayer, self).__init__()
        self.transition_layer = torch.nn.Sequential(
            torch.nn.BatchNorm2d(inplace),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(in_channels=inplace, out_channels=plance, kernel_size=1, stride=1, padding=0, bias=False),
...
class _DenseLayer(torch.nn.Module):
    def __init__(self, inplace, growth_rate, bn_size, drop_rate=0):
        ...
        self.dense_layer = torch.nn.Sequential(
            torch.nn.BatchNorm2d(inplace),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(in_channels=inplace, out_channels=bn_size * growth_rate, kernel_size=1, stride=1, padding=0,
                      bias=False),
            torch.nn.BatchNorm2d(bn_size * growth_rate),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(in_channels=bn_size * growth_rate, out_channels=growth_rate, kernel_size=3, stride=1, padding=1,
                      bias=False),
        )
