...
def Conv1x1BNReLU(in_channels, out_channels, groups):
    return torch.nn.Sequential(
        torch.nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1, groups=groups),
        torch.nn.BatchNorm2d(out_channels),
        torch.nn.ReLU6(inplace=True)
    )
...
class ShuffleNetUnits(torch.nn.Module):
    def __init__(self, in_channels, out_channels, stride, groups):
        ...
        if self.stride > 1:
            self.shortcut = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
class ShuffleNet(torch.nn.Module):
    def __init__(self, planes, layers, groups, num_classes=1000):
        ...
        self.global_pool = torch.nn.AvgPool2d(kernel_size=7, stride=1)
        self.dropout = torch.nn.Dropout(p=0.2)
        self.linear = torch.nn.Linear(in_features=planes[2] * 7 * 7, out_features=num_classes)
