...
class AgreementRouting(torch.nn.Module):
    def __init__(self, input_caps, output_caps, n_iterations):
        ...
        self.b = torch.nn.Parameter(torch.zeros((input_caps, output_caps)))

    def forward(self, u_predict):
        batch_size, input_caps, output_caps, output_dim = u_predict.size()

        c = torch.nn.functional.softmax(self.b)
        ...
        if self.n_iterations > 0:
            ...
            for r in range(self.n_iterations):
                ...
                c = torch.nn.functional.softmax(b_batch.view(-1, output_caps)).view(-1, input_caps, output_caps, 1)
...
class CapsLayer(torch.nn.Module):
    def __init__(self, input_caps, input_dim, output_caps, output_dim, routing_module):
        ...
        self.weights = torch.nn.Parameter(torch.Tensor(input_caps, input_dim, output_caps * output_dim))
...
class PrimaryCapsLayer(torch.nn.Module):
    def __init__(self, input_channels, output_caps, output_dim, kernel_size, stride):
        super(PrimaryCapsLayer, self).__init__()
        self.conv = torch.nn.Conv2d(input_channels, output_caps * output_dim, kernel_size=kernel_size, stride=stride)
...
class CapsNet(torch.nn.Module):
    def __init__(self, routing_iterations, n_classes=10):
        ...
        self.conv1 = torch.nn.Conv2d(1, 256, kernel_size=9, stride=1)
...
class ReconstructionNet(torch.nn.Module):
    def __init__(self, n_dim=16, n_classes=10):
        ...
        self.fc1 = torch.nn.Linear(n_dim * n_classes, 512)
        self.fc2 = torch.nn.Linear(512, 1024)
        self.fc3 = torch.nn.Linear(1024, 784)
        self.n_dim = n_dim
