...
def ConvBNReLUFactorization(in_channels, out_channels, kernel_sizes, paddings):
    return torch.nn.Sequential(
        torch.nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_sizes, stride=1,
                        padding=paddings),
        torch.nn.BatchNorm2d(out_channels),
        torch.nn.ReLU6(inplace=True)
    )
...
class InceptionV2ModuleA(torch.nn.Module):
...
class InceptionV2ModuleB(torch.nn.Module):
    def __init__(self, in_channels, out_channels1, out_channels2reduce, out_channels2, out_channels3reduce,
                 out_channels3, out_channels4):
...
class InceptionV2ModuleC(torch.nn.Module):
    def __init__(self, in_channels, out_channels1, out_channels2reduce, out_channels2, out_channels3reduce,
                 out_channels3, out_channels4):
...
class InceptionV3ModuleD(torch.nn.Module):
    def __init__(self, in_channels, out_channels1reduce, out_channels1, out_channels2reduce, out_channels2):
        ...
        self.branch3 = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
class InceptionV2(torch.nn.Module):
    def __init__(self, num_classes=1000, stage='train'):
        ...
        self.max_pool = torch.nn.MaxPool2d(kernel_size=7, stride=1)
