...
def softmax(input, dim=1):
    ...
    softmaxed_output = torch.nn.functional.softmax(transposed_input.contiguous().view(-1, transposed_input.size(-1)), dim=-1)
...
class CapsuleLayer(torch.nn.Module):
    def __init__(self, num_capsules, num_route_nodes, in_channels, out_channels, kernel_size=None, stride=None,
        ...
        if num_route_nodes != -1:
            self.route_weights = torch.nn.Parameter(torch.randn(num_capsules, num_route_nodes, in_channels, out_channels))
        else:
            self.capsules = torch.nn.ModuleList(
                [torch.nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=0) for _ in
                 range(num_capsules)])
