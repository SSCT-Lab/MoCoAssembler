...
def ConvBNActivation(in_channels, out_channels, kernel_size, stride, activate):
    return torch.nn.Sequential(
        ...
        torch.nn.ReLU6(inplace=True) if activate == 'relu' else HardSwish(inplace=True)
    )
...
class MDConv(torch.nn.Module):
    def __init__(self, nchannels, kernel_sizes, stride):
        ...
        for i in range(self.groups):
            self.layers.append(torch.nn.Conv2d(in_channels=self.split_channels[i], out_channels=self.split_channels[i],
                                         kernel_size=kernel_sizes[i], stride=stride, padding=int(kernel_sizes[i] // 2),
...
class SqueezeAndExcite(torch.nn.Module):
    def __init__(self, nchannels, squeeze_channels, se_ratio=1):
        ...
        self.SEblock = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=nchannels, out_channels=squeeze_channels, kernel_size=1, stride=1, padding=0),
            torch.nn.ReLU6(inplace=True),
            torch.nn.Conv2d(in_channels=squeeze_channels, out_channels=nchannels, kernel_size=1, stride=1, padding=0),
            torch.nn.Sigmoid(),
        )
...
class MDConvBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes, stride, expand_ratio, activate='relu', se_ratio=1):
        ...
        self.md_conv = torch.nn.Sequential(
...
class MixNet(torch.nn.Module):
    ...
    def __init__(self, type='mixnet_s'):
        ...
        self.stem = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=3, out_channels=stem_channels, kernel_size=3, stride=2, padding=1),
