...
class YOLOv3(object):
    ...
    def yolo_detection_block(self, input, num_filters, k=2, name=None):
        ...
        for j in range(k):
            ...
            conv = self.conv_bn_layer(conv, num_filters * 2, filter_size=3, stride=1, padding=1,
                                      name=yolo_name + str(j) + 'conv')
        route = self.conv_bn_layer(conv, num_filters, filter_size=1, stride=1, padding=0,
                                   name=yolo_name + str(j) + 'route')
        tip = self.conv_bn_layer(route, num_filters * 2, filter_size=3, stride=1, padding=1,
                                 name=yolo_name + str(j) + 'tip')
        return route, tip
    ...
    def net(self, input):
        ...
        for i, block in enumerate(blocks):
            ...
            # 为了跨视域链接，差值方式提升特征图尺寸
            if i < len(blocks) - 1:
                route = self.conv_bn_layer(route, 128 // (2 ** i), filter_size=1, stride=1, padding=0,
                                           name='route' + str(i))
