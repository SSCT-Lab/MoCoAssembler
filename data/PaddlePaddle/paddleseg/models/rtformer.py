...
class RTFormer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.seghead = SegHead(
            base_chs * 4, int(head_channels * 2), num_classes, lr_mult=lr_mult,need_proj = True)
    ...
    def forward(self, x):
        ...
        if self.training:
            x_out,embed = self.seghead(paddle.concat([x5_, x6], axis=1))  # 4c, 1/8
        else:
        ...
        if self.training:
            logit_list[0] = {"seg":logit_list[0],"embed":embed,'seg_contra':x_out}
...
class SegHead(nn.Layer):
    def __init__(self, in_channels, inter_channels, out_channels, lr_mult, proj_dim=720, need_proj = False):
        super().__init__()
        ...
        self.attn = ESEAttn(inter_channels)
        self.need_proj = need_proj
        if self.need_proj:
            self.proj_head = ProjectionHead(
                dim_in=inter_channels, proj_dim=proj_dim)

    def forward(self, x):
        x = self.conv1(self.relu(self.bn1(x)))
        x = x + self.attn(x)
        if self.need_proj and self.training:
