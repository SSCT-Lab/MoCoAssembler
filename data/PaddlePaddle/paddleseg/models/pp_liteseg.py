...
from .rtformer import ESEAttn,ProjectionHead
...
class PPLiteSegHead(nn.Layer):
    ...
    def __init__(self, backbone_out_chs, arm_out_chs, cm_bin_sizes, cm_out_ch,
        ...
        self.fusion_layer = ConvBNSilu(sum(arm_out_chs),arm_out_chs[0],3,1)
    ...
    def forward(self, in_feat_list):
        ...
        fusion_feat = [F.interpolate(out_feat_list[i],size=out_feat_list[0].shape[2:],
                                    mode='bilinear',align_corners=False) \
                       for i in range(1,len(out_feat_list))]
        fusion_feat.insert(0,out_feat_list[0])
        fusion_feat = self.fusion_layer(paddle.concat(fusion_feat,axis=1))
        out_feat_list[0] = fusion_feat
...
class SegHead(nn.Layer):
    def __init__(self, in_chan, mid_chan, n_classes):
        ...
        self.att = ESEAttn(mid_chan)
        self.dcn = DeformableConvV2(mid_chan,mid_chan,3)
...
class DeformableConvV2(nn.Layer):
    def __init__(self,
        ...
        self.bn = layers.SyncBatchNorm(out_channels, data_format='NCHW')
