...
def ghost_conv(x, out_channels, kernel_size, stride, kernel_regularizer=None):
    x1 = tf.keras.layers.Conv2D(out_channels // 2, kernel_size=kernel_size, strides=stride, padding="same",
                       use_bias=False, kernel_regularizer=kernel_regularizer)(x)
    x2 = tf.keras.layers.BatchNormalization(epsilon=1e-5)(x1)
    x2 = tf.keras.layers.Activation(tf.keras.activations.elu)(x2)
    x2 = tf.keras.layers.DepthwiseConv2D(kernel_size=(3, 3), strides=1, padding="same",
...
def fused_mbconv(x, in_channels, out_channels, kernel_size, activation, stride=1, reduction_ratio=4,
    ...
    x = tfa.tf.keras.layers.StochasticDepth()([shortcut, x])
    return x
