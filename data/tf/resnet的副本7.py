...
def resnet18(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    ...
    out = resnet(img_input, [2, 2, 2, 2], res_basic_block, classes, include_top, normalize=normalize,
                 activation=tf.keras.activations.relu)
...
def resnet34(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = resnet(img_input, [3, 4, 6, 3], res_basic_block, classes, include_top, normalize=normalize,
                 activation=tf.keras.activations.relu)
    model = tf.keras.Model(img_input, out)
...
def resnet50(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = resnet(img_input, [3, 4, 6, 3], res_bottleneck_block, classes, include_top, normalize=normalize,
                 activation=tf.keras.activations.relu)
    model = tf.keras.Model(img_input, out)
...
def resnet101(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
...
def resnet152(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = resnet(img_input, [3, 8, 36, 3], res_bottleneck_block, classes, include_top, normalize=normalize,
                 activation=tf.keras.activations.relu)
    model = tf.keras.Model(img_input, out)
...
def resnext50_32x4d(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = resnet(img_input, [3, 4, 6, 3], res_bottleneck_block, classes, include_top, group_size=32, base_width=4,
                 normalize=normalize, activation=tf.keras.activations.relu)
    model = tf.keras.Model(img_input, out)
...
def resnext101_32x8d(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = resnet(img_input, [3, 4, 23, 3], res_bottleneck_block, classes, include_top, group_size=32, base_width=8,
                 normalize=normalize, activation=tf.keras.activations.relu)
    model = tf.keras.Model(img_input, out)
...
def wide_resnet50_2(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = resnet(img_input, [3, 4, 6, 3], res_bottleneck_block, classes, include_top, base_width=128,
                 normalize=normalize, activation=tf.keras.activations.relu)
    model = tf.keras.Model(img_input, out)
...
def wide_resnet101_2(input_tensor=None, input_shape=None, classes=1000, include_top=True, weights="imagenet"):
    if input_tensor is None:
        img_input = tf.keras.layers.Input(shape=input_shape)
    else:
        if not tf.keras.backend.is_keras_tensor(input_tensor):
    ...
    out = resnet(img_input, [3, 4, 23, 3], res_bottleneck_block, classes, include_top, base_width=128,
                 normalize=normalize, activation=tf.keras.activations.relu)
    model = tf.keras.Model(img_input, out)
