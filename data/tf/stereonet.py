...
def soft_argmin(cost_volume, grid_size):
    ...
    disp_grid = tf.repeat(tf.repeat(tf.repeat(disp_grid, cost_volume.shape[0], axis=0), cost_volume.shape[2], axis=2), cost_volume.shape[3], axis=3)
...
class EdgeAwareRefinement(tf.keras.models.Model):
    ...
    def call(self, disp, colour, **kwargs):
...
def stereonet():
    return StereoNet(k=4, candidate_disparities=192)
