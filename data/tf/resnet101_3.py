...
class BasicBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, out_channel, strides=1, downsample=None, **kwargs):
        ...
        self.conv1 = tf.keras.layers.Conv2D(out_channel, kernel_size=3, strides=strides,
                                            padding="SAME", use_bias=False)
        self.bn1 = tf.keras.layers.BatchNormalization(momentum=0.9, epsilon=1e-5)

        self.conv2 = tf.keras.layers.Conv2D(out_channel, kernel_size=3, strides=1,
                                            padding="SAME", use_bias=False)
...
class Bottleneck(tf.keras.layers.Layer):
    ...
    def __init__(self, out_channel, strides=1, downsample=None, **kwargs):
        ...
        self.conv2 = tf.keras.layers.Conv2D(out_channel, kernel_size=3, use_bias=False,
...
def _make_layer(block, in_channel, channel, block_num, name, strides=1):
    ...
    if strides != 1 or in_channel != channel * block.expansion:
        downsample = Sequential([
            tf.keras.layers.Conv2D(channel * block.expansion, kernel_size=1, strides=strides,
                                   use_bias=False, name="conv1"),
            tf.keras.layers.BatchNormalization(momentum=0.9, epsilon=1.001e-5, name="BatchNorm")
        ], name="shortcut")
...
def _resnet(block, blocks_num, im_width=224, im_height=224, num_classes=1000, include_top=True):
    ...
    x = tf.keras.layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name="conv1/BatchNorm")(x)
...
def resnet101_3(im_width=224, im_height=224, num_classes=1000, include_top=True):
