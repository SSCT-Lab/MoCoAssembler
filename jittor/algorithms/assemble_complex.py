...
class Assembler_Complex:
    ...
    def assemble_code_tree(self):
        ...
        for sentence in execute_list:
            ...
            next_layer_count = 0
            with alive_bar(last_layer_count, bar='filling', title='generation'+str(generation-1)) as bar:
                for i in range(last_layer_count):
                    temp_model = model_queue.get()
                    file_name = temp_model.model.assemble_file(generation=temp_model.generation, test_dim=temp_model.index)
