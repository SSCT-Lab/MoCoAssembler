...
def get_similarity_dict() -> dict:
    path = os.path.join(file_paths.LAYER_SIMILARITY_PATH, 'layer_similarity.yaml')
    f = open(path, 'r', encoding='utf-8')
    d_all = yaml.full_load(f)
    f.close()
    api_list = []
    result = {}
    ...
    for api_name in list(d_all.keys()):
        d_now = d_all[api_name]
        ...
        if '1d' in api_name:
            ...
            result[api_name] = d_now
            continue
        modified_d_now = {}
        for name in d_now:
            if not (pool[0] in name or pool[1] in name):
                modified_d_now[name] = d_now[name]
            else:
                continue
        result[api_name] = modified_d_now
    return result
...
class Mutator:
    def __init__(self):  # 1 list for all api names, 2 dict for similarity and constraint, key api_name to access
        ...
        del temp
        self.api_similarity = get_similarity_dict()
        self.api_constraint = {}
    ...
    def api_para_mutate(self, api: str) -> (str, str):
        ...
        value, choice_type = get_value(dic=now_api_para_data[param_to_mutate], para_name=param_to_mutate)
    ...
    def api_name_mutate(self, api: str) -> (str, str):
        ...
        for param_name in required_list:
            if param_name not in new_para_dict.keys():
                new_para_dict[param_name] = get_value(params_constraint_dict[param_name], param_name)[0]
...
if __name__ == '__main__':
    m = Mutator()
    api1 = 'jittor.nn.Sigmoid()'
    api2 = 'jittor.nn.Conv2d(in_channels=64, out_channels=192, kernel_size=3, stride=1, padding=1)'
    api3 = 'jittor.nn.MaxPool2d(kernel_size=3, stride=2)'
    api4 = 'jittor.nn.Linear(in_features=25088, out_features=4096)'
    api5 = 'jittor.nn.AdaptiveAvgPool2d(output_size=6)'


    def test(api: str):
        for i in range(100):
            print(m.api_mutate(api))
