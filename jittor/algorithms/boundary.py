...
from assemble_complex_v2 import Model
import file_paths
import os
import sys
...
class Boundary_Runner:
    ...
    def generate_all_boundary_dec(self, dec: str) -> list[(str, int)]:  # dec, expect: 0:F 1:T 2:DEPEND
        function = get_function(dec)
        para_dict = get_params(dec)
        ...
        for para in para_list:
            ...
            if 'dtype' not in constraints[para].keys():
                ...
                if 'int' in constraints[para]['dtype']:
                    for value in int_pool:
                        new_para_dict[para] = value[0]
                        boundary_dec_list.append((generate_line(function, new_para_dict), value[1]))
                if 'tuple' in constraints[para]['dtype']:
                    tuple_pool = []
                    for i in int_pool:
                        for j in int_pool:
                            t = (i[0], j[0])
...
if __name__ == '__main__':
    _dec = "jittor.nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, padding=0, stride=1, dilation=1)"
