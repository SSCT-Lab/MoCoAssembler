...
class Assembler_Complex:
    def __init__(self, model_name):
        ...
        self.boundary_runner = boundary.Boundary_Runner(model_name)
        ...
        for sentence in execute_list:
            ...
            with alive_bar(last_layer_count, bar='smooth',
                ...
                for i in range(last_layer_count):
                    ...
                    if run_flag:
                        # boundary here
                        if temp_model.mutate_info not in ['sequence mutate', 'child model mutate', 'no mutate']:
                            self.boundary_runner.boundary_test(temp_model)
                        else:
                            pass
            ...
            # print(str(len(model_list)) + " succeed")
            ...
            for i in range(count):
                ...
                for j in range(self.n):
                    ...
                    # ==================================================================================================
                    if temp.mutate_info not in mutate_info_list:
                        mutate_info_list.append(temp.mutate_info)
                        model_queue.put(temp)
                        next_layer_count += 1
                    else:
                        if random.randint(1, 10) <= 5:  # another method to cut branches
                            model_queue.put(temp)
                            next_layer_count += 1

            self.dfc(os.path.join(file_paths.MUTATED_MODEL_PATH, self.seed_model.net_name))
            generation = generation + 1
            self.boundary_runner.conv_count = 0
