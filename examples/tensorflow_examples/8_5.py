...
# lenet hidden layer
    ...
    x = keras.layers.ZeroPadding2D(padding=4)(x)
    x = keras.layers.MaxPool2D(pool_size=1, strides=2)(x)
    x = keras.layers.Conv2D(filters=32, kernel_size=5, activation="relu", padding="same", use_bias=False)(x)
    x = keras.layers.MaxPool2D(pool_size=2, strides=1)(x)
    x = keras.layers.Flatten(data_format="channels_last")(x)
    x = keras.layers.Dense(units=200, activation="linear")(x)
# lenet output layer
